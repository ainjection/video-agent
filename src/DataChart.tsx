...
export const dataChartSchema = z.object({
  type: z.enum(['bar', 'line', 'pie', 'donut']).default('bar'),
  ...
  style: z.enum(['minimal', 'gradient', 'neon', 'glass']).default('minimal'),
  color: z.string().default('#8b5cf6'),
  showValues: z.boolean().default(true),
  showLabels: z.boolean().default(true),
});

type DataChartProps = z.infer<typeof dataChartSchema>;

const defaultData = [
  { label: 'Jan', value: 65, color: '#8b5cf6' },
  { label: 'Feb', value: 85, color: '#ec4899' },
  { label: 'Mar', value: 45, color: '#f97316' },
              ...
              {segments.map((segment, i) => {
                ...
                return (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, opacity: legendOpacity }}>
                    ...
                    />
                    <span style={{ color: '#ffffff', fontSize: 16, fontFamily: 'system-ui, sans-serif' }}>{segment.label}</span>
                    {showValues && (
                      <span style={{ color: '#71717a', fontSize: 14, fontFamily: 'system-ui, sans-serif' }}>({segment.percentage}%)</span>
                    )}
                  </div>
                );
              })}
